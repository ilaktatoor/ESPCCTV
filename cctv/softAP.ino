void sendFrameToServer() {
  ...
  HTTPClient http;
  String serverUrl = String("http://") + String(user_wifi.server_host) + ":" + String(user_wifi.port) + "/video-frame/" + String(user_wifi.camid);
...
void checkResetButton() {
  ...
  if (digitalRead(RESET_PIN) == LOW) {
    ...
    strcpy(user_wifi.ssid, "");  // Sets ssid to empty string
    EEPROM.put(0, user_wifi);
    EEPROM.commit();
    delay(1000);
    ESP.restart();
